...
/**
 * @file test_logic.js
 * @description モバイルキャリア診断アルゴリズムのオフライン検証用テストスクリプト (Node.js環境用)
 * 
 * 【主な役割・機能】
 * 1. 診断ロジックのオフライン再現
 *    - ブラウザ（script.js）側の診断計算ロジックをNode.js環境に抽出し、UI無しで検証を行います。
 * 2. データ検証
 *    - JSONデータ（plans.json、rules.json）をローカルファイルシステムから読み込み、整合性をテストします。
 * 3. テストシナリオの実行
 *    - ライトユーザー、ヘビーユーザー、家族割複数回線、MNP等の代表的な5つのユースケースをシミュレート。
 *    - 各シナリオにおける「料金」「品質」「サービス」の判定結果を出力し、期待される動作か確認。
 * 
 * 【データ連携・ファイル依存】
 * - data/plans.json : 各キャリアの料金プラン情報をテスト用にロード
 * - data/rules.json : 各キャリアの割引ルール定義をテスト用にロード
 * 
 * 【使用方法】
 * コマンドラインより以下を実行：
